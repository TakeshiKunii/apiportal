...
(function () {
  ...
  // Retuen default content type according to the API type
  function getDefaultContentType() {
    if (window.apiType && window.apiType == 'wsdl') {
      return 'text/xml';
    } else {
  ...
  SwaggerResource.prototype.addOperations = function (resource_path, ops, consumes, produces) {
    if (ops) {
      ...
      for (var i = 0; i < ops.length; i++) {
        ...
        var op = new SwaggerOperation(o.nickname, resource_path, method, o.parameters, o.summary, o.notes, type, responseMessages, this, consumes, produces, o.authorizations, o.documentationUrl);
  ...
  var SwaggerOperation = function (nickname, path, method, parameters, summary, notes, type, responseMessages, resource, consumes, produces, authorizations, documentationUrl) {
    var _this = this;
    ...
    for (var i = 0; i < this.parameters.length; i++) {
      ...
      if (type === 'array') {
        if (param.items) {
          if (param.items.$ref) {
            type = 'array[' + param.items.$ref + ']';
          } else if (param.items.type) {
            type = 'array[' + param.items.type + ']';
          } else {
            type = 'array[string]';
          }
        } else {
  ...
  SwaggerOperation.prototype.getSampleJSON = function (type, models) {
    ...
    listType = this.isListType(type);
    isPrimitive = ((listType != null) && models[listType]) || (models[type] != null) ? false : true;
    val = isPrimitive ? void 0 :
        ((listType != null) ? models[listType].createJSONSample() : models[type].createJSONSample());
    if (val) {
  ...
  SwaggerOperation.prototype["do"] = function (args, opts, callback, error) {
    ...
    // clone args to used them in Swagger request
  ...
  SwaggerOperation.prototype.pathify = function (args) {
    var path = this.pathJson();
    var params = this.parameters;
    for (var i = 0; i < params.length; i++) {
      var param = params[i];
      if (param.paramType === 'path') {
        if (args[param.name]) {
          // apply path params and remove from args
          var reg = new RegExp('\\{\\s*?' + param.name + '.*?\\}(?=\\s*?(\\/|$))', 'gi');
          path = path.replace(reg, this.encodePathParam(args[param.name]));
          delete args[param.name];
        }
        else
          throw "" + param.name + " is a required path param.";
  ...
  var SwaggerRequest = function (type, url, params, opts, successCallback, errorCallback, operation, execution) {
    ...
    if (!((this.headers != null) && (this.headers.mock != null))) {
      ...
      if (opts.mock == null) {
        if (status !== false) {
          // add only resource path to the obj
          obj.resourcePath = operation.pathify(operation.args);
          new SwaggerHttp().execute(obj);
        } else {
          obj.canceled = true;
        }
  ...
  SwaggerRequest.prototype.setHeaders = function (params, operation) {
    ...
    if (consumes && this.operation.consumes.length > 0) {
      if (this.operation.consumes.indexOf(consumes) === -1) {
        // Case when either the user has created their own entry in the request content-type drop-down
        // or we've set the consumes to either "multipart/form-data" or "application/x-www-form-urlencoded"
        log("server doesn't consume " + consumes + ", try " + JSON.stringify(this.operation.consumes));

        // Check if there's at least one consumes content type defined in the swagger/wsdl definition
        if (this.operation.consumes[0]) {
          consumes = this.operation.consumes[0];
        // If there isn't, set default request content type - text/xml for WSDLs and application/json for Swaggers.
        } else {
          consumes = getDefaultContentType()
        }
      }
    }
    ...
    if (accepts && this.operation.produces) {
      if (this.operation.produces.indexOf(accepts) === -1) {
        // Like for consumes
        // Case when either the user has created their own entry in the response content-type drop-down
        log("server can't produce " + accepts);

        // Check if there's at least one produces content type defined in the swagger/wsdl definition
        if (this.operation.produces[0]) {
          accepts = this.operation.produces[0];
          // If there isn't, set default response content type - text/xml for WSDLs and application/json for Swaggers.
        } else {
          accepts = getDefaultContentType();
        }
      }
    }
  ...
  ShredHttpClient.prototype.execute = function (obj) {
    ...
    var transform = function (response) {
      ...
      delete out.headers["Server"];
      delete out.headers["Host"];
      delete out.headers["Dnt"];
      delete out.headers["Referer"];
      delete out.headers["Cookie"];
      delete out.headers["Max-Forwards"];
      delete out.headers["Access-Control-Allow-Origin"];
      delete out.headers["Date"];
      delete out.headers["Strict-Transport-Security"];
      delete out.headers["X-XSS-Protection"];
      delete out.headers["X-Frame-Options"];
      delete out.headers["X-Xss-Protection"];
      delete out.headers["X-Content-Type-Options"];
    ...
    /**
     * If 'Try It' proxy is enabled change default obj.url with the proxy host.
     * add path (to the resource) and http method as params
     */
    if (window.swaggerUi.options.tryItProxy) {
      // save real url to use it later
      window.APIPortal = {};
      // save the original uri for the resource
      window.APIPortal.tryItUrl = obj.url;
      // change destination to the API Portal proxy
//      console.log('here');
//console.log(window.swaggerUi.options.staticFeed.securityProfile.devices[0]);
//console.log('there'); 
      if ( typeof(window.swaggerUi.options.staticFeed.securityProfile.devices[0]) !== 'undefined' && typeof(window.swaggerUi.options.staticFeed.securityProfile.devices[0].takeFrom) !== 'undefined' && window.swaggerUi.options.staticFeed.securityProfile.devices[0].takeFrom === "QUERY" && window.swaggerUi.options.staticFeed.securityProfile.devices[0].name == "API Key") {
          if (obj.resourcePath.indexOf("?") == -1){             
                obj.resourcePath += '?' + window.swaggerUi.options.staticFeed.securityProfile.devices[0].keyField + '=' + window.swaggerUi.options.apiKeyValue;
            }
            else{
                obj.resourcePath += '&' + window.swaggerUi.options.staticFeed.securityProfile.devices[0].keyField + '=' + window.swaggerUi.options.apiKeyValue;
            }
          obj.url = window.swaggerUi.options.tryItProxy + '&path=' + encodeURIComponent(obj.resourcePath);
      }
      else {
      obj.url = window.swaggerUi.options.tryItProxy + '&path=' + encodeURIComponent(obj.resourcePath)+ '&base64='+$('#globalRedeToken option:selected').val();
        }
    }
